...
(define (%quick-sort! v elt< start end)
  (let recur ((l start) (r end))	; Sort the range [l,r).
	...
	(let* ((median (lambda (v1 v2 v3)
			 (receive (little big)
			     (if (_elt< v1 v2) (values v1 v2) (values v2 v1))
			   (if (_elt< big v3) big
			       (if (_elt< little v3) v3 little)))))
	       (pivot (_median (vector-ref v l)
			      (vector-ref v (quotient (+ l r) 2))
			      (vector-ref v (- r 1)))))

	  (let loop ((i l) (j (- r 1)))
	    (let ((i (let scan ((i i)) (if (_elt< (vector-ref v i) pivot)
					   (scan (+ i 1))
					   i)))
		  (j (let scan ((j j)) (if (_elt< pivot (vector-ref v j))
					   (scan (- j 1))
					   j))))
	      (if (< i j)
